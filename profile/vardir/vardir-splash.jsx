// vardir-splash.jsx — Vardir brand splash.
// Reuses the reveal choreography (spark → assembly → wedge sweep → lockup) but
// runs standalone: self-driving clock (no scrubber/playback bar) and a
// viewport-COVER scaler so it fills any screen edge-to-edge and stays responsive.
// Brand-corrected per Vardir guidelines: Space Grotesk + Space Mono, indigo
// label color, Norwegian-first tagline label.
// Reads TimelineContext, useTime, Easing, animate, clamp from window (animations.jsx).

const { TimelineContext, useTime, Easing, animate, clamp, Stage } = window;

// ── Brand palette (Vardir guidelines) ───────────────────────────────────────
const BG     = '#15181D';   // Ink
const WHITE  = '#FFFFFF';
const PURPLE = '#5856E0';   // Indigo
const LABEL  = '#9D9BF0';   // light indigo — mono label on Ink (per guidelines)

// ── Themes: recolor the whole reveal per the three brand backgrounds ────────
const THEMES = {
  Ink: {
    bg: '#15181D', ring: '#FFFFFF', wedge: '#5856E0', word: '#FFFFFF', label: '#9D9BF0',
    grid: 'rgba(255,255,255,0.05)', echo: 'rgba(255,255,255,0.04)',
    glow: 'rgba(88,86,224,0.5)', pulse: 'rgba(88,86,224,0.16)',
    shockA: 'rgba(255,255,255,0.6)', shockB: 'rgba(88,86,224,0.8)',
    vignette: 'radial-gradient(120% 90% at 50% 46%, rgba(255,255,255,0.03), rgba(0,0,0,0) 55%), radial-gradient(90% 90% at 50% 60%, rgba(0,0,0,0) 55%, rgba(0,0,0,0.5))',
  },
  Indigo: {
    bg: '#3B39B0', ring: '#FFFFFF', wedge: '#C7C6F7', word: '#FFFFFF', label: 'rgba(255,255,255,0.80)',
    grid: 'rgba(255,255,255,0.07)', echo: 'rgba(255,255,255,0.07)',
    glow: 'rgba(190,188,255,0.45)', pulse: 'rgba(255,255,255,0.13)',
    shockA: 'rgba(255,255,255,0.7)', shockB: 'rgba(255,255,255,0.55)',
    vignette: 'radial-gradient(120% 90% at 50% 46%, rgba(255,255,255,0.06), rgba(0,0,0,0) 55%), radial-gradient(92% 92% at 50% 62%, rgba(0,0,0,0) 50%, rgba(16,14,72,0.55))',
  },
  Paper: {
    bg: '#EFEEE9', ring: '#15181D', wedge: '#5856E0', word: '#15181D', label: '#5856E0',
    grid: 'rgba(21,24,29,0.06)', echo: 'rgba(21,24,29,0.05)',
    glow: 'rgba(88,86,224,0.22)', pulse: 'rgba(88,86,224,0.10)',
    shockA: 'rgba(21,24,29,0.35)', shockB: 'rgba(88,86,224,0.7)',
    vignette: 'radial-gradient(120% 90% at 50% 46%, rgba(255,255,255,0.4), rgba(0,0,0,0) 55%), radial-gradient(95% 95% at 50% 60%, rgba(0,0,0,0) 58%, rgba(21,24,29,0.10))',
  },
};
...
function Comet({ t, th }) {
  ...
  const dir = [Math.cos(Math.PI / 6), -Math.sin(Math.PI / 6)];
  const D = 780;
  const posAt = (tt) => {
  ...
  const ghosts = [0.018, 0.036, 0.055, 0.075].map((dt, i) => {
    ...
    return <circle key={i} cx={gx} cy={gy} r={5 - i} fill={th.wedge} opacity={0.4 - i * 0.09} />;
  });
  ...
  return (
    <g opacity={fade}>
      ...
      <circle cx={x} cy={y} r={11} fill={th.wedge} opacity={0.35} />
    </g>
  );
}
...
function Mark({ t, th }) {
  const segs = BANDS.map((b, j) => {
    ...
    if (p <= 0) return null;
    const rot = (Easing.easeOutBack(p) - 1) * 42;
    const ghostRot = rot * 1.45;
    return (
      <g key={j}>
        {p < 1 && (
          <polygon points={ptsStr(b.pts)} fill={th.ring} opacity={0.28 * (1 - p)}
            transform={`rotate(${ghostRot.toFixed(3)} ${CX} ${CY})`} />
        )}
        <polygon points={ptsStr(b.pts)} fill={th.ring} opacity={clamp(p * 2.2, 0, 1)}
    ...
    wedgeEl = (
      <g>
        ...
        <g clipPath="url(#wedgeClip)">
          <polygon points={ptsStr(WEDGE.pts)} fill={th.wedge}
        ...
        {wp < 1 && (() => {
          ...
          return (
            <g>
              <line x1={CX} y1={CY} x2={lx} y2={ly} stroke={th.wedge} strokeWidth="9" opacity={0.3 * (1 - wp)} strokeLinecap="round" />
  ...
  if (t > T.impact && t < w0 + 0.14) {
    ...
    ember = (
      <g opacity={inO * out}>
        <circle cx={CX} cy={CY} r={r * 2.6} fill={th.wedge} opacity="0.22" />
  ...
  const flash = fl > 0.02 ? (
    <circle cx={CX} cy={CY} r={26 + 60 * (1 - fl)} fill={th.ring} opacity={fl * 0.8} />
  ) : null;
  ...
  const sparks = (sb > 0 && sb < 1) ? [0, 1, 2, 3, 4, 5, 6, 7].map((i) => {
    ...
    return <circle key={i} cx={sx} cy={sy} r={(1 - sb) * 3} fill={i % 3 ? th.wedge : '#CFCEFF'} opacity={(1 - sb) * 0.9} />;
  }) : null;
  ...
  return (
    <svg viewBox="288 288 448 448" width="100%" height="100%" style={{ display: 'block', overflow: 'visible' }}>
      <HexShock t={t} t0={T.impact} dur={0.62} color={th.shockA} maxScale={1.7} />
      <HexShock t={t} t0={T.lock} dur={0.72} color={th.shockB} maxScale={2.1} />
      {segs}
      {wedgeEl}
...
function Scene({ tagline, theme }) {
  const t = useTime();
  const th = THEMES[theme] || THEMES.Ink;
  const showTag = tagline && tagline !== 'None';
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, overflow: 'hidden', background: th.bg }}>
      ...
      <div style={{
        ...
        backgroundImage: `radial-gradient(${th.grid} 1px, transparent 1px)`,
      ...
      <svg width="1920" height="1080" viewBox="0 0 1920 1080" style={{ position: 'absolute', inset: 0, opacity: gridIn }}>
        {[300, 400, 520].map((R, i) => (
          <polygon key={i}
            ...
            fill="none" stroke={th.echo} strokeWidth="1.5"
      ...
      <div style={{
        ...
        background: `radial-gradient(circle, ${th.pulse}, rgba(0,0,0,0) 60%)`,
        opacity: gridPulse, pointerEvents: 'none',
      }} />

      <div style={{
        position: 'absolute', inset: 0,
        background: th.vignette,
      }} />
      ...
      <div style={{ position: 'absolute', inset: 0, transform: `scale(${cam.toFixed(4)})`, transformOrigin: '960px 512px' }}>
        ...
        <div style={{
          ...
          background: `radial-gradient(circle, ${th.glow}, rgba(0,0,0,0) 62%)`,
        ...
        {showTag && (
          <div style={{
            ...
            fontSize: 20, color: th.label, textTransform: 'uppercase',
            letterSpacing: `${(0.36 - 0.12 * tagE).toFixed(3)}em`, paddingLeft: '0.36em',
...
// ── Self-driving clock: plays through, holds the lockup, loops ───────────────
function Clock({ children }) {
  const [t, setT] = React.useState(0);
  React.useEffect(() => {
    let raf, startTs = null;
    const HOLD = 1.6;
    const LOOP = T.dur + HOLD;
    const tick = (now) => {
      if (startTs == null) startTs = now;
      const el = ((now - startTs) / 1000) % LOOP;
      setT(Math.min(el, T.dur));
  ...
  return (
    <TimelineContext.Provider value={{ time: t, duration: T.dur, playing: true }}>
      {children}
    </TimelineContext.Provider>
  );
}

// ── Responsive shell: COVER-scale the 1920×1080 world to fill any viewport ───
function VardirSplash(props) {
  const tagline = props && props.tagline != null ? props.tagline : 'Web · App · Programvare';
  const theme = (props && props.theme) || 'Ink';
  const th = THEMES[theme] || THEMES.Ink;
  ...
  React.useEffect(() => {
    ...
    if (!el) return;
    const measure = () => {
      setScale(Math.max(el.clientWidth / 1920, el.clientHeight / 1080));
    };
    measure();
  ...
  return (
    <div ref={ref} style={{ position: 'fixed', inset: 0, overflow: 'hidden', background: th.bg }}>
      <div style={{
        ...
        <Clock><Scene tagline={tagline} theme={theme} /></Clock>
      </div>
    </div>
  );
}

window.VardirSplash = VardirSplash;

// ── 16:9 export harness: same reveal inside the animation Stage (exportable) ─
function VardirRevealStage(props) {
  const theme = (props && props.theme) || 'Ink';
  const tagline = props && props.tagline != null ? props.tagline : 'None';
  const th = THEMES[theme] || THEMES.Ink;
  return React.createElement(
    Stage,
    { width: 1920, height: 1080, duration: T.dur, background: th.bg, persistKey: 'vardir-splash-export' },
    React.createElement(Scene, { tagline, theme })
  );
}
window.VardirRevealStage = VardirRevealStage;
